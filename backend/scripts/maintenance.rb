...
class DatabaseMaintenance
  ...
  def self.show_stats
    ...
    puts "  Access logs (last 7 days): #{DB[:access_logs].where(Sequel.lit('accessed_at > ?', Time.now - (7 * 24 * 3600))).count}"
    puts "  Pending reset tokens: #{DB[:password_reset_tokens].where(Sequel.lit('expires_at > ? AND used = ?', Time.now, false)).count}"
